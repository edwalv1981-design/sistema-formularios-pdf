...
async function deleteSignedForm(id) {
    ...
    try {
        ...
        showCustomModal('Error', 'No se pudo eliminar el archivo', 'error');
    }
}
...
async function updateManualExpiration(docId, dateValue) {
    if (!dateValue) return;
    try {
        const response = await fetch(`/api/documentacion-personal/${docId}/manual-date`, {
            method: 'PATCH',
            headers: { 
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}` 
            },
            body: JSON.stringify({ fecha_expiracion: dateValue })
        });
        
        if (response.ok) {
            fetchPersonalDocs(); // Refrescar para ver el nuevo estado calculado
        } else {
            showCustomModal('Error', 'No se pudo guardar la fecha manual', 'error');
        }
    } catch (err) {
        console.error(err);
    }
...
function toggleEditDate(docId) {
    const container = document.getElementById(`date-container-${docId}`);
    container.innerHTML = `<div style="display:flex; align-items:center; gap:5px;">
        <input type="date" id="date-input-${docId}" style="padding:4px; font-size:10px; background:rgba(0,0,0,0.2); border:1px solid var(--primary); border-radius:4px; color:var(--text-main);">
        <button class="btn-primary" onclick="updateManualExpiration('${docId}', document.getElementById('date-input-${docId}').value)" style="padding:4px 8px; font-size:10px; border-radius:4px;"><i class="ph ph-check"></i></button>
    </div>`;
}
...
function showUploadPersonalModal() {
    const html = `
        <div style="display:flex; flex-direction:column; gap:20px;">
            ...
            <div class="form-group">
                <label style="color:white; display:block; margin-bottom:10px;">Archivo (PDF o Imagen)</label>
                <input type="file" id="doc-personal-file" accept=".pdf, .jpg, .jpeg, .png, .webp" style="width:100%; color:white;">
            </div>
...
async function uploadPersonalFile() {
    ...
    const type = document.getElementById('doc-personal-type').value;
    if(!fileInput.files[0]) return alert('Adjunte un archivo');
    ...
    try {
        const response = await fetch('/api/documentacion-personal/upload', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            body: formData
        });
...
async function deletePersonalDoc(id) {
    ...
    try {
        ...
        showCustomModal('Error', 'Falla al eliminar', 'error');
    }
}
...
function showEditPersonalModal(id, currentType) {
    const html = `
        <div style="display:flex; flex-direction:column; gap:20px;">
            ...
            <div class="form-group">
                <input type="file" id="doc-edit-file" style="color:white;">
            </div>
            <button class="btn-primary" onclick="reemplazarDocumento('${id}', '${currentType}')" style="width:100%">Recargar Documento</button>
        </div>
    `;
    showCustomModal('Reemplazar Documento', html, 'info');
}

async function reemplazarDocumento(id, type) {
